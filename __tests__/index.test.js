...
describe('Response tag', () => {
  ...
  describe('JSONPath', () => {
    ...
    it('basic query eval override', async () => {
      const requests = [{ _id: 'req_1', parentId: 'wrk_1' }];
      ...
      const context = _genTestContext(requests, responses);
      const result = await tag.run(context, 'body', 'req_1', '$.foo', `"override"`);

      expect(result).toBe('override');
    })

    it('basic query eval pass-through', async () => {
      const requests = [{ _id: 'req_1', parentId: 'wrk_1' }];
      ...
      const context = _genTestContext(requests, responses);
      const result = await tag.run(context, 'body', 'req_1', '$.foo', `output`);

      expect(result).toBe('bar');
    })

    it('basic query eval no filter', async () => {
      const requests = [{ _id: 'req_1', parentId: 'wrk_1' }];
